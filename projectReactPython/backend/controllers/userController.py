...
def getAllProduct():
    try:
        ...
        data = userModel.allProduct(limit=limit, page=page)
        if data is None:
            return jsonify({'error': 'Gagal mengambil data list Produk...'}), 500
        return jsonify(data)
...
def getProdukRekomendasi():
    data = userModel.listProdukRekomendasi()
    if data is None:
        return jsonify({'error': 'Gagal mengambil data list Produk Rekomendasi...'}), 500
    return jsonify(data)
        # HANYA 15 PRODUK REKOMENDASI 
@userBp.route('/listTopProdukRekomendasi', methods=['GET'])
def getTopProdukRekomendasi():
    data = userModel.listProdukRekomendasi()
    if data is None:
        return jsonify({'error': 'Gagal mengambil data list Produk Rekomendasi...'}), 500
